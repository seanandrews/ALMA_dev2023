...
pWSU_MS = 'TRUTH_PERFECT.ALMA-WSU_native.bin8x'
dBLC_MS = 'ALMA-BLC_122kHz_SAMPLED'
tBLC_MS = 'TRUTH.ALMA-BLC_122kHz'
pBLC_MS = 'TRUTH_PERFECT.ALMA-BLC_122kHz'
...
pWSU_dict = read_MS(sdir+pWSU_MS+'.REGRID'+ig+'.ms')
pBLC_dict = read_MS(sdir+pBLC_MS+'.REGRID'+ig+'.ms')
...
pBLC_vel = 1e-3 * sc.c * (1 - pBLC_dict['0'].nu_LSRK[_stamp,:] / nu0)
pWSU_vel = 1e-3 * sc.c * (1 - pWSU_dict['0'].nu_LSRK[_stamp,:] / nu0)

# true visibility spectrum
tru_vis = tru_dict['0'].vis[0,:,ix]
tBLC_vis = tBLC_dict['0'].vis[0,:,ix]
tWSU_vis = tWSU_dict['0'].vis[0,:,ix]
pBLC_vis = pBLC_dict['0'].vis[0,:,ix]
pWSU_vis = pWSU_dict['0'].vis[0,:,ix]
...
# BLC
ax[0,1].plot(tBLC_vel, tBLC_vis.real, '-', color=tru_col, lw=1, alpha=0.5)
ax[0,1].plot(tru_vel, tru_vis.real, '-', color='xkcd:gunmetal')
...
for im in range(len(imeth)):
    ...
    ax[1,1].plot(int_vel, int_vis.real - pBLC_vis.real, 
                 'o', ms=3, color=cols[im], label=nms[im])


# WSU
ax[0,0].plot(tWSU_vel, tWSU_vis.real, '-', color=tru_col, lw=1, alpha=0.5)
ax[0,0].plot(tru_vel, tru_vis.real, '-', color='xkcd:gunmetal')
...
for im in range(len(imeth)):
    ...
    # plot the fractional interpolate residuals
    ax[1,0].plot(int_vel, int_vis.real - pWSU_vis.real, 
...
fig.savefig('figs/demo_sregrid_PERFECT_top.pdf')
fig.clf()
