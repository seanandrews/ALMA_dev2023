...
tfile_BLC = sdir+'TRUTH_PERFECT.ALMA-BLC_122kHz.REGRID'+ig
dfile_BLC = sdir+'ALMA-BLC_122kHz_SAMPLED.REGRID'+ig
tfile_WSU = sdir+'TRUTH_PERFECT.ALMA-WSU_native.bin8x.REGRID'+ig
dfile_WSU = sdir+'ALMA-WSU_native_SAMPLED.bin8x.REGRID'+ig

# frequency selections
nch = 5
ix0_blc, dch_blc = 46, 3
...
for ia in range(len(imeth)):
    ...
    for i in range(nch):
        ...
        rim = ax.imshow(rTb_blc, origin='lower', cmap=mymap, extent=ext,
...
lax.text(0.07, 0.015, 'WSU, x8 (108 kHz)', ha='left', va='center', 
         transform=lax.transAxes, fontsize=8.5, color='k')
lax.axis('off')

lax = fig.add_axes([0.505, 0.92, 0.2, 0.8], alpha=0)
lax.text(0.07, 0.015, 'BLC (122 kHz)', ha='left', va='center',
         transform=lax.transAxes, fontsize=8.5, color='k')
lax.axis('off')



figo = 'figs/demo_residcubes_PERFECT'
